...
namespace HoloToolkit.Unity
{
    ...
    /// </summary>
    public class StabilizationPlaneModifierModified : Singleton<StabilizationPlaneModifierModified>
    {
        ...
        [Tooltip("Use the Main Camera to set the plane to the gazed upon hologram. If disabled, the plane will always be at a constant distance.")]
        public bool UseMainCameraNormal = true;
        ...
        private void ConfigureGazeManagerPlane(float deltaTime)
        {
            ...
            {
                 gazeOrigin = GazeManager.Instance.GazeOrigin;
                 gazeDirection = GazeManager.Instance.GazeNormal;
            }
        ...
        private void OnDrawGizmos()
        {
            if (UnityEngine.Application.isPlaying && DrawGizmos)
            {
                ...
                Vector3 focalPlaneNormal;
                if (UseMainCameraNormal)
                {
                    focalPlaneNormal = Camera.main.transform.forward;
                }
                else
                {
                    focalPlaneNormal = GazeManager.Instance.GazeNormal;
                }
